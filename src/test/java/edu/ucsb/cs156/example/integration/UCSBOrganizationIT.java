...
public class UCSBOrganizationIT {
  ...
  public void an_admin_user_can_post_a_new_organization() throws Exception {
    ...
    MvcResult response =
        mockMvc
            .perform(
                post("/api/ucsborganization/post?orgCode=AS&orgTranslationShort=Associated Students&orgTranslation=Associated Students UCSB&inactive=false")
                    .with(csrf()))
            .andExpect(status().isOk())
            .andReturn();
  ...
  public void an_admin_user_can_delete_an_organization() throws Exception {
    ...
    MvcResult response =
        mockMvc
            .perform(delete("/api/ucsborganization?orgCode=AS").with(csrf()))
            .andExpect(status().isOk())
            .andReturn();

    // assert
    String responseString = response.getResponse().getContentAsString();
    assert responseString.contains("Organization with code AS deleted");
  }
}
