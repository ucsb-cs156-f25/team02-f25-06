...
public class UCSBDiningCommonsMenuItemIT {
  ...
  @WithMockUser(roles = {"USER"})
  @Test
  public void a_user_can_delete_a_menu_item() throws Exception {
    // arrange
    UCSBDiningCommonsMenuItem menuItem =
        UCSBDiningCommonsMenuItem.builder()
            .diningCommonsCode("ortega")
    ...
    assertEquals(1, ucsbDiningCommonsMenuItemRepository.count());

    // act
    mockMvc
        .perform(delete("/api/ucsbdiningcommonsmenuitem?id=" + itemId).with(csrf()))
        .andExpect(status().isOk())
        .andReturn();

    // assert
    assertEquals(0, ucsbDiningCommonsMenuItemRepository.count());
  }

  @WithMockUser(roles = {"USER"})
  @Test
  public void a_user_can_post_a_new_menu_item() throws Exception {
    // arrange
    UCSBDiningCommonsMenuItem expectedMenuItem =
        UCSBDiningCommonsMenuItem.builder()
            .id(1L)
            .diningCommonsCode("ortega")
            .name("Pancakes")
