...
describe("UCSBOrganizationIndexPage tests", () => {
  ...
  test("renders three UCSB organizations correctly for regular user", async () => {
    ...
    const orgCode = screen.getByText("AS");
    expect(orgCode).toBeInTheDocument();

    const orgTranslationShort = screen.getByText("Associated Students");
    expect(orgTranslationShort).toBeInTheDocument();

    const orgTranslation = screen.getByText("Associated Students UCSB");
  ...
  test("what happens when you click delete, admin", async () => {
    ...
    axiosMock
      ...
      .reply(200, "UCSB Organization with orgCode AS was deleted");
    ...
    await waitFor(() => {
      expect(mockToast).toBeCalledWith(
        "UCSB Organization with orgCode AS was deleted",
      );
    });
