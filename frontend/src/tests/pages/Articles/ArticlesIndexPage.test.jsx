...
describe("ArticlesIndexPage tests", () => {
  ...
  test("renders three articles correctly for regular user", async () => {
    ...
    const title = screen.getByText("Team02 Instructions");
    expect(title).toBeInTheDocument();

    const url = screen.getByText(
      "https://ucsb-cs156.github.io/f25/lab/team02.html",
    );
    expect(url).toBeInTheDocument();

    const explanation = screen.getByText("Where we share our progress");
    expect(explanation).toBeInTheDocument();

    // const email = screen.getByText("natalieforte@ucsb.edu");
    // expect(email).toBeInTheDocument();

    const dateAdded = screen.getByText("2025-10-29T19:12:21");
